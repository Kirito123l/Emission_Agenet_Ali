"""Pure response-shaping helpers extracted from ``api.routes``.

These helpers are intentionally kept small and framework-light so the first
safe extraction pass from ``api/routes.py`` does not affect route registration
or request handling behavior.
"""
