...
DISPERSION_POLLUTANT_CONFIG_PATH = Path(__file__).resolve().parents[1] / "config" / "dispersion_pollutants.yaml"
DEFAULT_DISPERSION_SUPPORTED_POLLUTANTS = {
    "NOx": {"status": "supported", "science_caveats": []},
    "CO": {"status": "supported", "science_caveats": []},
    "PM2.5": {"status": "supported", "science_caveats": []},
    "PM10": {"status": "supported", "science_caveats": []},
    "CO2": {"status": "experimental", "science_caveats": []},
    "THC": {"status": "experimental", "science_caveats": []},
}
...
class UnifiedRouter:
    ...
    def __init__(self, session_id: str, memory_storage_dir: Optional[str | Path] = None):
        ...
        self._live_dispersion_clarification: Dict[str, Any] = {
            "active_kind": None,
            "decision": None,
            "base_arguments": {},
            "original_user_message": None,
    ...
    def _ensure_live_dispersion_clarification_bundle(self) -> Dict[str, Any]:
        if (
            not hasattr(self, "_live_dispersion_clarification")
            or not isinstance(self._live_dispersion_clarification, dict)
        ):
            self._live_dispersion_clarification = {
                "active_kind": None,
                "decision": None,
                "base_arguments": {},
                "original_user_message": None,
            }
        return self._live_dispersion_clarification
    ...
    def to_persisted_state(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "live_state": {
                ...
                "dispersion_clarification": self._json_safe_payload(
                    self._ensure_live_dispersion_clarification_bundle()
                ),
            },
        }
    ...
    def restore_persisted_state(self, payload: Dict[str, Any]) -> None:
        ...
        dispersion_clarification = live_state.get("dispersion_clarification")
        if isinstance(dispersion_clarification, dict):
            self._ensure_live_dispersion_clarification_bundle().update(dispersion_clarification)
    ...
    def _prepare_tool_arguments(
        ...
        if state is not None:
            ...
            if tool_name in {"calculate_dispersion", "analyze_hotspots", "render_spatial_map"}:
                hints = self._extract_message_execution_hints(state)
                if hints.get("pollutants") and not effective_arguments.get("pollutant"):
                    user_pollutant = list(hints["pollutants"])[0]
                    effective_arguments["pollutant"] = user_pollutant
        ...
        requested_pollutant = effective_arguments.get("pollutant")
        lookup_layer_type = layer_type
        if tool_name == "render_spatial_map" and requested_pollutant and not layer_type:
            lookup_layer_type = "dispersion"
        stored_result = context_store.get_result_for_tool(
            tool_name,
            label=scenario_label,
            layer_type=lookup_layer_type,
            pollutant=requested_pollutant,
        )
        ...
        if requested_pollutant and tool_name in {"render_spatial_map", "analyze_hotspots"}:
            spatial_data = spatial if isinstance(spatial, dict) else {}
            spatial_query = spatial_data.get("query_info", {}) if isinstance(spatial_data.get("query_info"), dict) else {}
            spatial_pollutant = spatial_query.get("pollutant") or spatial_data.get("pollutant")
            if spatial_pollutant and self._pollutant_key(spatial_pollutant) != self._pollutant_key(requested_pollutant):
                logger.info(
                    "%s: skipped legacy spatial fallback because requested pollutant=%s but memory pollutant=%s",
                    tool_name,
                    requested_pollutant,
                    spatial_pollutant,
                )
    ...
    def _extract_pollutants_from_message(self, message: str) -> List[str]:
        return self._extract_pollutants_from_text(message)

    @staticmethod
    def _pollutant_key(pollutant: Any) -> str:
        return str(pollutant or "").strip().lower()

    @staticmethod
    def _unique_pollutants(pollutants: List[Any]) -> List[str]:
        seen: set[str] = set()
        ordered: List[str] = []
    ...
    def _extract_emission_pollutants_from_result(self, result_payload: Optional[Dict[str, Any]]) -> List[str]:
        """Extract pollutants available in a macro-emission result payload."""
        ...
        if isinstance(query_info, dict) and isinstance(query_info.get("pollutants"), list):
            candidates.extend(query_info["pollutants"])

        summary = data.get("summary", {})
        if isinstance(summary, dict):
            totals = summary.get("total_emissions_kg_per_hr")
            if isinstance(totals, dict):
                candidates.extend(totals.keys())
        ...
        return self._unique_pollutants(candidates)

    def _load_dispersion_pollutant_config(self) -> Dict[str, Dict[str, Any]]:
        cached = getattr(self, "_dispersion_pollutant_config_cache", None)
        if isinstance(cached, dict):
            return cached

        config: Dict[str, Dict[str, Any]] = {
            pollutant: dict(details)
            for pollutant, details in DEFAULT_DISPERSION_SUPPORTED_POLLUTANTS.items()
        }
        try:
            with DISPERSION_POLLUTANT_CONFIG_PATH.open("r", encoding="utf-8") as handle:
                payload = yaml.safe_load(handle) or {}
            loaded = payload.get("pollutants", {}) if isinstance(payload, dict) else {}
            if isinstance(loaded, dict) and loaded:
                config = {
                    str(pollutant): dict(details or {})
                    for pollutant, details in loaded.items()
                    if str(pollutant).strip()
                }
        except Exception:
            logger.warning(
                "Failed to load dispersion pollutant config from %s; using defaults",
                DISPERSION_POLLUTANT_CONFIG_PATH,
                exc_info=True,
            )

        setattr(self, "_dispersion_pollutant_config_cache", config)
        return config

    def _build_dispersion_pollutant_decision(
        self,
        state: TaskState,
        arguments: Optional[Dict[str, Any]] = None,
        requested_pollutants: Optional[List[str]] = None,
    ) -> Dict[str, Any]:
        computed = self._available_emission_pollutants_for_state(state, arguments)
        config = self._load_dispersion_pollutant_config()
        config_by_key = {
            self._pollutant_key(pollutant): (pollutant, details)
            for pollutant, details in config.items()
        }
        computed_by_key = {self._pollutant_key(pollutant): pollutant for pollutant in computed}
        requested = self._unique_pollutants(requested_pollutants or [])
        candidate_pollutants = requested if requested else computed

        eligible: List[str] = []
        unsupported: List[str] = []
        unavailable: List[str] = []
        details_by_pollutant: Dict[str, Dict[str, Any]] = {}

        for pollutant in candidate_pollutants:
            key = self._pollutant_key(pollutant)
            if requested and computed and key not in computed_by_key:
                unavailable.append(pollutant)
                continue

            config_entry = config_by_key.get(key)
            if config_entry is None:
                unsupported.append(pollutant)
                details_by_pollutant[pollutant] = {
                    "status": "unsupported",
                    "science_caveats": ["No dispersion applicability entry is configured."],
                }
                continue

            canonical, details = config_entry
            status = str(details.get("status") or "supported").strip().lower()
            details_by_pollutant[canonical] = dict(details)
            if status in {"unsupported", "disabled", "not_supported"}:
        ...
        return {
            "computed": computed,
            "eligible": self._unique_pollutants(eligible),
            "unsupported": self._unique_pollutants(unsupported),
            "unavailable": self._unique_pollutants(unavailable),
            "requested": requested,
            "details": details_by_pollutant,
        }

    def _format_dispersion_pollutant_notice(
        self,
        decision: Dict[str, Any],
        *,
        action: str,
    ) -> str:
        computed = list(decision.get("computed") or [])
        eligible = list(decision.get("eligible") or [])
        unsupported = list(decision.get("unsupported") or [])
        unavailable = list(decision.get("unavailable") or [])
        parts: List[str] = []

        if computed:
            parts.append(f"上一轮排放结果包含：{'、'.join(computed)}。")
        if eligible:
            if action == "batch":
                parts.append(f"本次将按同一扩散场景依次分析：{'、'.join(eligible)}。")
            elif action == "clarify":
                parts.append(f"当前可直接做物理扩散分析的污染物有：{'、'.join(eligible)}。")
            else:
                parts.append(f"本次将分析：{'、'.join(eligible)}。")
        if unsupported:
            parts.append(f"暂不自动分析：{'、'.join(unsupported)}。")
        if unavailable:
            parts.append(f"请求的污染物尚未在上一轮排放结果中计算：{'、'.join(unavailable)}。")
        if unsupported:
            parts.append(
                "未自动分析的污染物不是被静默丢弃；原因是当前系统未配置其物理扩散适用性，"
                "不是因为排放量低或可以忽略。"
            )
        return "\n".join(parts).strip()

    def _build_no_eligible_dispersion_pollutants_message(self, decision: Dict[str, Any]) -> str:
        notice = self._format_dispersion_pollutant_notice(decision, action="none")
        if notice:
            return f"{notice}\n\n当前没有可用于本次物理扩散分析的污染物，因此我不会调用扩散计算。"
        return "当前没有找到上一轮排放结果中的可扩散污染物，因此我不会调用扩散计算。"

    def _set_dispersion_pollutant_notice(
        self,
        state: TaskState,
        notice: str,
    ) -> None:
        ...
        if not notice:
            return
        existing = str(getattr(state, "_dispersion_pollutant_notice", "") or "").strip()
        if notice in existing:
            return
        setattr(
            state,
            "_dispersion_pollutant_notice",
            f"{existing}\n{notice}".strip() if existing else notice,
        )

    @staticmethod
    def _decision_has_skipped_pollutants(decision: Dict[str, Any]) -> bool:
        return bool(decision.get("unsupported") or decision.get("unavailable"))

    def _append_dispersion_pollutant_notice(
        self,
        state: TaskState,
        response_text: str,
    ) -> str:
        notice = str(getattr(state, "_dispersion_pollutant_notice", "") or "").strip()
        text = str(response_text or "")
        if not notice or notice in text:
            return text
        return f"{notice}\n\n{text}" if text else notice

    def _available_emission_pollutants_for_state(
        self,
        state: TaskState,
        arguments: Optional[Dict[str, Any]] = None,
    ) -> List[str]:
        arguments = arguments if isinstance(arguments, dict) else {}
        direct = self._extract_emission_pollutants_from_result(arguments.get("_last_result"))
        if direct:
            return direct

        scenario_label = arguments.get("scenario_label")
        stored = self._ensure_context_store().get_result_for_tool(
            "calculate_dispersion",
            label=scenario_label,
        )
        stored_pollutants = self._extract_emission_pollutants_from_result(stored)
        if stored_pollutants:
            return stored_pollutants

        try:
            fact_mem = self.memory.get_fact_memory()
            memory_pollutants = self._extract_emission_pollutants_from_result(
                {"success": True, "data": fact_mem.get("last_spatial_data")}
            )
            if memory_pollutants:
                return memory_pollutants
        except Exception:
            pass

        for item in reversed(state.execution.tool_results or []):
            if not isinstance(item, dict):
                continue
            if item.get("name") != "calculate_macro_emission":
                continue
            result = item.get("result")
            pollutants = self._extract_emission_pollutants_from_result(result)
            if pollutants:
                return pollutants
        return []

    @staticmethod
    def _is_all_pollutants_request(message: str) -> bool:
        ...
        return any(
            token in message_lower
            for token in ("所有污染物", "全部污染物", "每个污染物", "各污染物", "逐个污染物")
        ) or (
            "所有已计算污染物" in message_lower
            or ("所有" in message_lower and "污染物" in message_lower and "逐个" in message_lower)
            or ("污染物" in message_lower and "逐个扩散" in message_lower)
        )

    @staticmethod
    def _is_all_pollutants_selection_reply(message: str) -> bool:
        normalized = re.sub(r"[\s。.!！?？,，、]+", "", str(message or "").strip().lower())
        if not normalized:
            return False
        # Explicit single-pollutant replies must stay single-selection even when
        # they contain verbs such as "看" or "分析".
        if UnifiedRouter._extract_pollutants_from_text(message):
            return False
        if normalized in {
            "全部",
            "全都",
            ...
            return True
        has_all_signal = any(token in normalized for token in ("全部", "所有", "全都"))
        has_collective_do_signal = "都" in normalized and any(
            token in normalized
            for token in ("看", "要", "做", "分析", "扩散", "算", "跑")
        )
        if has_all_signal or has_collective_do_signal:
            return True
        return UnifiedRouter._is_all_pollutants_request(message)

    @staticmethod
    def _extract_pollutants_from_text(message: str) -> List[str]:
        ...
        for alias, normalized in sorted(
            ...
            if not alias_text or normalized in seen:
                continue
            position = UnifiedRouter._find_alias_position(message_lower, alias_text.lower())
            if position is None:
                continue
            seen.add(normalized)
    ...
    def _resolve_pollutant_selection_reply(
        self,
        message: str,
        eligible: List[str],
    ) -> List[str]:
        ...
        if not eligible:
            return []
        if self._is_all_pollutants_selection_reply(message):
            return eligible

        requested = self._extract_pollutants_from_message(message)
        if not requested:
            return []
        eligible_by_key = {self._pollutant_key(pollutant): pollutant for pollutant in eligible}
        selected = [
            eligible_by_key[self._pollutant_key(pollutant)]
            for pollutant in requested
            if self._pollutant_key(pollutant) in eligible_by_key
        ]
        return self._unique_pollutants(selected)

    def _clear_live_dispersion_clarification_state(self) -> None:
        bundle = self._ensure_live_dispersion_clarification_bundle()
        bundle["active_kind"] = None
        bundle["decision"] = None
        bundle["base_arguments"] = {}
        bundle["original_user_message"] = None

    def _activate_dispersion_pollutant_selection_state(
        self,
        state: TaskState,
        decision: Dict[str, Any],
        *,
        base_arguments: Optional[Dict[str, Any]] = None,
    ) -> None:
        bundle = self._ensure_live_dispersion_clarification_bundle()
        bundle["active_kind"] = "pollutant_selection"
        bundle["decision"] = self._json_safe_payload(decision) or dict(decision)
        bundle["base_arguments"] = self._json_safe_payload(base_arguments or {}) or {}
        bundle["original_user_message"] = state.user_message

    def _build_dispersion_tool_calls_for_pollutants(
        self,
        pollutants: List[str],
        *,
        base_arguments: Optional[Dict[str, Any]] = None,
        id_prefix: str = "dispersion",
        source: str = "batch",
    ) -> List[ToolCall]:
        tool_calls: List[ToolCall] = []
        base = dict(base_arguments or {})
        base.pop("pollutant", None)
        base.pop("_pollutant_source", None)
        base.pop("_batch_pollutant_dispatch", None)
        for index, pollutant in enumerate(self._unique_pollutants(pollutants)):
            arguments = dict(base)
            arguments["pollutant"] = pollutant
            arguments["_pollutant_source"] = source
            if len(pollutants) > 1 or source == "batch":
                arguments["_batch_pollutant_dispatch"] = True
            tool_calls.append(
                ToolCall(
    ...
    def _activate_tool_calls_from_dispersion_clarification(
        self,
        state: TaskState,
        tool_calls: List[ToolCall],
        *,
        ...
        state._llm_response = LLMResponse(
            content="",
            tool_calls=tool_calls,
            finish_reason="tool_calls",
        )
        state.execution.selected_tool = tool_calls[0].name if tool_calls else None
        self._capture_tool_call_parameters(state, tool_calls)
        if len(tool_calls) > 1:
            setattr(state, "_allow_repeated_dispersion_batch", True)
        if trace_obj:
            ...
            trace_obj.record(
                ...
                input_summary={
                    "resolved_pollutants": [
                        tool_call.arguments.get("pollutant")
    ...
    def _handle_active_dispersion_clarification(
        self,
        state: TaskState,
        trace_obj: Optional[Trace] = None,
    ) -> bool:
        bundle = self._ensure_live_dispersion_clarification_bundle()
        if bundle.get("active_kind") != "pollutant_selection":
            return False
        decision = bundle.get("decision") if isinstance(bundle.get("decision"), dict) else {}
        eligible = list(decision.get("eligible") or [])
        selected = self._resolve_pollutant_selection_reply(state.user_message or "", eligible)
        if not selected:
            self._set_direct_user_response_state(
                state,
                self._build_dispersion_pollutant_selection_prompt_from_decision(decision),
                stage=TaskStage.NEEDS_CLARIFICATION,
                stage_before=TaskStage.INPUT_RECEIVED,
                reason="Active dispersion pollutant clarification remained unresolved",
                trace_obj=trace_obj,
        ...
        selected_decision["eligible"] = selected
        if self._decision_has_skipped_pollutants(decision):
            notice = self._format_dispersion_pollutant_notice(selected_decision, action="batch" if len(selected) > 1 else "single")
            self._set_dispersion_pollutant_notice(state, notice)
        base_arguments = bundle.get("base_arguments") if isinstance(bundle.get("base_arguments"), dict) else {}
        self._clear_live_dispersion_clarification_state()
        tool_calls = self._build_dispersion_tool_calls_for_pollutants(
            selected,
            base_arguments=base_arguments,
            id_prefix="clarified_dispersion",
            source="batch" if len(selected) > 1 else "user",
        )
        self._activate_tool_calls_from_dispersion_clarification(
            state,
            tool_calls,
            trace_obj=trace_obj,
            reasoning="Resolved active dispersion pollutant selection",
        )
        return True

    def _maybe_start_dispersion_pollutant_clarification(
        self,
        state: TaskState,
        *,
        trace_obj: Optional[Trace] = None,
        ...
        if "calculate_dispersion" not in desired_chain:
            return False
        all_request = self._is_all_pollutants_request(state.user_message or "")
        if hints.get("pollutants") and not all_request:
            return False
        completed_tools = list(state.execution.completed_tools or [])
        if "calculate_macro_emission" not in completed_tools and not self._ensure_context_store().has_result("emission"):
            return False
        decision = self._build_dispersion_pollutant_decision(state)
        eligible = list(decision.get("eligible") or [])
        if decision.get("computed") and not eligible:
            self._set_direct_user_response_state(
                state,
                self._build_no_eligible_dispersion_pollutants_message(decision),
                stage=TaskStage.DONE,
                stage_before=TaskStage.INPUT_RECEIVED,
                reason="No eligible emission pollutants available for dispersion",
                trace_obj=trace_obj,
                trace_step_type=TraceStepType.SYNTHESIS,
            )
            return True
        if all_request and eligible:
            notice = self._format_dispersion_pollutant_notice(
                decision,
                action="batch" if len(eligible) > 1 else "single",
            )
            self._set_dispersion_pollutant_notice(state, notice)
            tool_calls = self._build_dispersion_tool_calls_for_pollutants(
                eligible,
                id_prefix="direct_all_dispersion",
                source="batch" if len(eligible) > 1 else "inferred",
            )
            self._activate_tool_calls_from_dispersion_clarification(
                state,
                tool_calls,
                trace_obj=trace_obj,
                reasoning="Resolved explicit all-pollutants dispersion request before meteorology confirmation",
            )
            return True
        if len(eligible) <= 1:
            return False
        self._activate_dispersion_pollutant_selection_state(state, decision)
        self._set_direct_user_response_state(
            state,
            self._build_dispersion_pollutant_selection_prompt_from_decision(decision),
            stage=TaskStage.NEEDS_CLARIFICATION,
            stage_before=TaskStage.INPUT_RECEIVED,
            reason="Multiple eligible emission pollutants must be selected before meteorology/execution",
            trace_obj=trace_obj,
            trace_step_type=TraceStepType.CLARIFICATION,
        )
        return True

    def _build_dispersion_pollutant_selection_prompt(self, pollutants: List[str]) -> str:
        options = "、".join(pollutants)
        return (
            "当前排放结果包含多个污染物，扩散计算需要明确目标污染物，避免默认只计算 NOx。\n\n"
            f"可用污染物：{options}\n\n"
            f"请指定要扩散的污染物，例如“计算 CO2 扩散”；也可以说“对所有污染物逐个扩散”。"
        )

    def _build_dispersion_pollutant_selection_prompt_from_decision(
        self,
        decision: Dict[str, Any],
    ) -> str:
        eligible = list(decision.get("eligible") or [])
        options = "、".join(eligible)
        notice = self._format_dispersion_pollutant_notice(decision, action="clarify")
        question = f"您要看全部，还是先看某一个？例如：{options}。" if options else ""
        return "\n\n".join(part for part in (notice, question) if part)

    def _latest_dispersion_result_payload(
        self,
        state: Optional[TaskState] = None,
    ) -> Optional[Dict[str, Any]]:
        if state is not None:
            for item in reversed(state.execution.tool_results or []):
                if not isinstance(item, dict) or item.get("name") != "calculate_dispersion":
                    continue
                result = item.get("result")
                if isinstance(result, dict) and result.get("success"):
                    return result

        stored = self._ensure_context_store().get_by_type("dispersion")
        if stored is not None and isinstance(stored.data, dict) and stored.data.get("success"):
            return stored.data
        return None

    def _extract_dispersion_scenario_arguments(
        self,
        dispersion_result: Optional[Dict[str, Any]],
    ) -> Dict[str, Any]:
        if not isinstance(dispersion_result, dict):
            return {}

        data = dispersion_result.get("data", dispersion_result)
        if not isinstance(data, dict):
            return {}

        query_info = data.get("query_info", {}) if isinstance(data.get("query_info"), dict) else {}
        meteorology_used = (
            data.get("meteorology_used", {})
            if isinstance(data.get("meteorology_used"), dict)
            else {}
        )
        inherited: Dict[str, Any] = {}

        source_mode = str(meteorology_used.get("_source_mode") or "").strip().lower()
        preset_name = meteorology_used.get("_preset_name")
        if source_mode == "sfc_file" and meteorology_used.get("path"):
            inherited["meteorology"] = meteorology_used.get("path")
        elif preset_name:
            inherited["meteorology"] = preset_name
            overrides = meteorology_used.get("_overrides", {})
            if isinstance(overrides, dict):
                for key, change in overrides.items():
                    if key in {"wind_speed", "wind_direction", "stability_class", "mixing_height"}:
                        value = change.get("to") if isinstance(change, dict) else meteorology_used.get(key)
                        if value is not None:
                            inherited[key] = value
        elif source_mode == "custom":
            inherited["meteorology"] = "custom"
            for key in ("wind_speed", "wind_direction", "stability_class", "mixing_height"):
                if meteorology_used.get(key) is not None:
                    inherited[key] = meteorology_used[key]

        if query_info.get("roughness_height") is not None:
            inherited["roughness_height"] = query_info["roughness_height"]
        if query_info.get("display_grid_resolution_m") is not None:
            inherited["grid_resolution"] = query_info["display_grid_resolution_m"]
        if query_info.get("contour_interp_resolution_m") is not None:
            inherited["contour_resolution"] = query_info["contour_interp_resolution_m"]
        if data.get("scenario_label"):
            inherited["scenario_label"] = data.get("scenario_label")
        return inherited

    def _inherit_dispersion_scenario_arguments(
        self,
        effective_arguments: Dict[str, Any],
        *,
        state: Optional[TaskState] = None,
    ) -> None:
        previous = self._latest_dispersion_result_payload(state)
        inherited = self._extract_dispersion_scenario_arguments(previous)
        if not inherited:
            return

        applied: Dict[str, Any] = {}
        for key, value in inherited.items():
            if key in effective_arguments or value is None:
                continue
            effective_arguments[key] = value
            applied[key] = value
        if applied:
            effective_arguments["_inherited_dispersion_scenario"] = applied
    ...
    def _build_missing_input_clarification(self, state: TaskState) -> Optional[str]:
        ...
        if next_tool == "calculate_dispersion" and "calculate_macro_emission" not in completed_tools:
            if self._ensure_context_store().has_result("emission"):
                pass
            elif state.file_context.has_file and str(state.file_context.task_type or "").strip() == "macro_emission":
                return None
            else:
                return "做扩散分析前，我需要路网排放结果。你可以上传路网文件让我先算排放，或直接说明要基于哪一份排放结果继续。"
        if next_tool == "calculate_dispersion" and not hints.get("pollutants"):
            decision = self._build_dispersion_pollutant_decision(state)
            eligible = list(decision.get("eligible") or [])
            if decision.get("computed") and not eligible:
                return self._build_no_eligible_dispersion_pollutants_message(decision)
            if len(eligible) > 1 and not self._is_all_pollutants_request(state.user_message or ""):
                self._activate_dispersion_pollutant_selection_state(state, decision)
                return self._build_dispersion_pollutant_selection_prompt_from_decision(decision)
    ...
    def _build_deterministic_fallback_tool_call(self, state: TaskState) -> Optional[LLMResponse]:
        ...
        if next_tool == "query_emission_factors":
            ...
            if hints.get("pollutants") and self._is_all_pollutants_request(state.user_message or ""):
                decision = self._build_dispersion_pollutant_decision(state, arguments)
                eligible = list(decision.get("eligible") or [])
                if not eligible and decision.get("computed"):
                    return None
                if eligible:
                    notice = self._format_dispersion_pollutant_notice(decision, action="batch")
                    self._set_dispersion_pollutant_notice(state, notice)
                    tool_calls = self._build_dispersion_tool_calls_for_pollutants(
                        eligible,
                        base_arguments=arguments,
                        id_prefix=f"deterministic_{next_tool}",
                        source="batch",
                    )
                    setattr(state, "_allow_repeated_dispersion_batch", True)
                    return LLMResponse(
            ...
            if hints.get("pollutants"):
                ...
                arguments["_pollutant_source"] = "user"
            else:
                decision = self._build_dispersion_pollutant_decision(state, arguments)
                eligible = list(decision.get("eligible") or [])
                if len(eligible) == 1:
                    arguments["pollutant"] = eligible[0]
                    arguments["_pollutant_source"] = "inferred"
                    if self._decision_has_skipped_pollutants(decision):
                        notice = self._format_dispersion_pollutant_notice(decision, action="single")
                        self._set_dispersion_pollutant_notice(state, notice)
                elif len(eligible) > 1 and self._is_all_pollutants_request(state.user_message or ""):
                    notice = self._format_dispersion_pollutant_notice(decision, action="batch")
                    self._set_dispersion_pollutant_notice(state, notice)
                    tool_calls = self._build_dispersion_tool_calls_for_pollutants(
                        eligible,
    ...
    @staticmethod
    def _has_successful_tool_results(tool_results: Optional[List[Dict[str, Any]]]) -> bool:
        return any(
    ...
    @staticmethod
    def _is_redundant_render_affordance(
        tool_name: str,
        affordance: Optional[ActionAffordance],
    ) -> bool:
        if tool_name != "render_spatial_map" or affordance is None:
            return False
        if affordance.status != ReadinessStatus.ALREADY_PROVIDED:
            return False
        artifact = affordance.provided_artifact
        artifact_id = str(getattr(artifact, "artifact_id", "") or "")
        return artifact_id.startswith("map:")

    def _record_redundant_render_skip(
        self,
        state: TaskState,
        tool_name: str,
        ...
        *,
        ...
        artifact = affordance.provided_artifact
        artifact_id = str(getattr(artifact, "artifact_id", "") or "")
        display_name = str(getattr(artifact, "display_name", "") or affordance.display_name)
        note = f"Skipped redundant {tool_name} because {artifact_id or display_name} was already delivered in this turn."
        skipped = getattr(state.execution, "_redundant_render_skips", None)
        if not isinstance(skipped, list):
            skipped = []
            setattr(state.execution, "_redundant_render_skips", skipped)
        skipped.append(
            {
                "tool_name": tool_name,
        ...
        state.append_plan_note(
            ...
            tool_name=None if planned_step is not None else tool_name,
            reconciliation=True,
        )
        ...
        if trace_obj:
            trace_obj.record(
                ...
                output_summary={
                    "skipped": True,
                    "reason_code": "redundant_render_already_provided",
                    "provided_artifact": (
                        artifact.to_dict()
    ...
    @staticmethod
    def _is_redundant_render_block_payload(blocked_info: Optional[Dict[str, Any]]) -> bool:
        if not isinstance(blocked_info, dict):
            return False
        if blocked_info.get("reason_code") != "artifact_already_provided":
            return False
        action_id = str(blocked_info.get("action_id") or "")
        if not action_id.startswith("render_"):
            return False
        provided_artifact = blocked_info.get("provided_artifact")
        artifact_id = ""
        if isinstance(provided_artifact, dict):
            artifact_id = str(provided_artifact.get("artifact_id") or "")
        return artifact_id.startswith("map:")

    def _expand_multi_pollutant_dispersion_calls(
        self,
        state: TaskState,
        tool_calls: Optional[List[Any]],
    ) -> List[Any]:
        """Expand an explicit "all pollutants" dispersion request into one call per pollutant."""
        calls = list(tool_calls or [])
        if not calls or not self._is_all_pollutants_request(state.user_message or ""):
            return calls

        expanded: List[Any] = []
        did_expand = False
        for tool_call in calls:
            if str(getattr(tool_call, "name", "")) != "calculate_dispersion":
            ...
            hints = self._extract_message_execution_hints(state)
            source = str(arguments.get("_pollutant_source") or "").strip().lower()
            if arguments.get("pollutant") and source == "batch":
                tool_call.arguments = arguments
                expanded.append(tool_call)
                continue
            if (
                arguments.get("pollutant")
                and hints.get("pollutants")
                and not self._is_all_pollutants_request(state.user_message or "")
                and source not in {"batch"}
            ):
                arguments["_pollutant_source"] = "user"
                tool_call.arguments = arguments
                expanded.append(tool_call)
                continue
            if arguments.get("pollutant") and not hints.get("pollutants"):
                arguments.pop("pollutant", None)
            ...
            self._set_dispersion_pollutant_notice(state, notice)
            expanded.extend(
                self._build_dispersion_tool_calls_for_pollutants(
                    eligible,
                    base_arguments=arguments,
                    id_prefix=str(getattr(tool_call, "id", "dispersion")),
                    source="batch",
                )
            )
    ...
    def _extract_key_stats(self, tool_name: str, data: Dict[str, Any]) -> str:
        ...
        query_info = data.get("query_info", {}) if isinstance(data.get("query_info"), dict) else {}
        if isinstance(summary, dict):
            concentration_unit = str(summary.get("unit") or query_info.get("unit") or "μg/m³")
            ...
            if "mean_concentration" in summary and isinstance(summary["mean_concentration"], (int, float)):
                stats.append(f"mean={summary['mean_concentration']:.4f} {concentration_unit}")
            if "max_concentration" in summary and isinstance(summary["max_concentration"], (int, float)):
                stats.append(f"max={summary['max_concentration']:.4f} {concentration_unit}")
    ...
    def _evaluate_missing_parameter_preflight(
        ...
        *,
        ...
        if tool_name == "calculate_dispersion":
            user_pollutants = list(hints.get("pollutants") or [])
            source = str(explicit_arguments.get("_pollutant_source") or "").strip().lower()
            requested = user_pollutants if user_pollutants else []
            decision = self._build_dispersion_pollutant_decision(
                state,
                explicit_arguments,
                requested_pollutants=requested,
            )
            eligible = list(decision.get("eligible") or [])
            ...
            if user_pollutants:
                if explicit_arguments.get("pollutant") and source in {"batch", "user"}:
                    explicit_arguments["_pollutant_source"] = source
                    return False
                if eligible:
                    explicit_arguments["pollutant"] = eligible[0]
                    explicit_arguments["_pollutant_source"] = "user"
                    return False
                return False

            if len(eligible) == 1:
                explicit_arguments["pollutant"] = eligible[0]
                explicit_arguments["_pollutant_source"] = source or "inferred"
                if self._decision_has_skipped_pollutants(decision):
                    notice = self._format_dispersion_pollutant_notice(decision, action="single")
                    self._set_dispersion_pollutant_notice(state, notice)
                return False

            if len(eligible) > 1 and self._is_all_pollutants_request(state.user_message or ""):
                return False

            if len(eligible) > 1 and source not in {"user", "batch"}:
                self._activate_dispersion_pollutant_selection_state(
                    state,
                    decision,
                    base_arguments=explicit_arguments,
                )
                self._set_direct_user_response_state(
                    state,
                    self._build_dispersion_pollutant_selection_prompt_from_decision(decision),
                    stage=TaskStage.NEEDS_CLARIFICATION,
                    stage_before=TaskStage.EXECUTING,
                    reason="Multiple eligible emission pollutants available before dispersion",
                    trace_obj=trace_obj,
                    trace_step_type=TraceStepType.CLARIFICATION,
                )
                return True
    ...
    def _reconcile_plan_before_execution(
        ...
        if next_step is None:
            if tool_name == "calculate_dispersion" and getattr(state, "_allow_repeated_dispersion_batch", False):
                result["matched"] = True
                result["note"] = "Repeated calculate_dispersion call allowed for multi-pollutant batch dispatch."
                return result
    ...
    async def _state_handle_input(
        ...
        if self._handle_active_dispersion_clarification(state, trace_obj=trace_obj):
            return
        ...
        if self._maybe_start_dispersion_pollutant_clarification(state, trace_obj=trace_obj):
            return
    ...
    async def _state_handle_executing(
        ...
        while current_response and current_response.tool_calls and not dependency_blocked and not repair_halted:
            ...
            pending_tool_calls: List[Any] = list(current_response.tool_calls)
            pending_index = 0
            while pending_index < len(pending_tool_calls):
                original_tool_call = pending_tool_calls[pending_index]
                expanded_tool_calls = self._expand_multi_pollutant_dispersion_calls(
                    state,
                    [original_tool_call],
                )
                if expanded_tool_calls != [original_tool_call]:
                    pending_tool_calls[pending_index : pending_index + 1] = expanded_tool_calls
                    continue

                tool_call = pending_tool_calls[pending_index]
                pending_index += 1
                ...
                effective_arguments.pop("_batch_pollutant_dispatch", None)
                ...
                if (
                    self._is_redundant_render_affordance(tool_call.name, readiness_affordance)
                    and self._has_successful_tool_results(state.execution.tool_results)
                ):
                    self._record_redundant_render_skip(
                        state,
                        tool_call.name,
    ...
    async def _state_build_response(
        ...
        if (
            state.execution.blocked_info
            and self._is_redundant_render_block_payload(state.execution.blocked_info)
            and self._has_successful_tool_results(state.execution.tool_results)
        ):
            state.execution.blocked_info = None
        ...
        if state.execution.tool_results:
            ...
            response_text = self._append_dispersion_pollutant_notice(state, response_text)
            chart_data = frontend_payloads["chart_data"]
            table_data = frontend_payloads["table_data"]
            map_data = frontend_payloads["map_data"]
            download_file = frontend_payloads["download_file"]
    ...
    async def _process_response(
        ...
        for tool_call in response.tool_calls:
            ...
            if tool_call.name == "calculate_dispersion" and not effective_arguments.get("pollutant"):
                available_pollutants = self._extract_emission_pollutants_from_result(
                    effective_arguments.get("_last_result")
                )
                if len(available_pollutants) == 1:
                    effective_arguments["pollutant"] = available_pollutants[0]
                elif len(available_pollutants) > 1:
                    return RouterResponse(
                        text=self._sanitize_response_text(
                            self._build_dispersion_pollutant_selection_prompt(available_pollutants)
                        ),
            ...
            if (
                self._is_redundant_render_affordance(tool_call.name, readiness_affordance)
                and self._has_successful_tool_results(tool_results)
            ):
                logger.info(
                    "Skipped redundant %s because map payload was already produced earlier in this execution round",
                    tool_call.name,
                )
                continue
    ...
    def clear_history(self):
        ...
        self._ensure_live_dispersion_clarification_bundle().update(
            {
                "active_kind": None,
                "decision": None,
                "base_arguments": {},
