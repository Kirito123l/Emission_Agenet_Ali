...
class ClarificationContract(BaseContract):
    ...
    def _stage2_system_prompt() -> str:
        return (
            ...
            "5. 同时判断工具意图，输出 intent: {resolved_tool, intent_confidence, reasoning, chain}。"
            "chain 是字符串数组，列出完整的多步工具执行序列（含当前工具和下游工具）。\n"
            ...
            "用户请求的工具如果需要上游结果（如 calculate_dispersion 需要 emission），"
            "且 available_results 中不包含对应结果类型，则 intent.chain 必须包含完整执行序列。"
            "示例：用户说\"分析路网排放并扩散\" → chain: [\"calculate_macro_emission\", \"calculate_dispersion\"]；"
            "用户说\"查排放因子\" → chain: [\"query_emission_factors\"]。"
            "chain[0] 始终是当前立即执行的工具，后续元素是依赖链上的下游工具。\n"
            ...
            "(因为信息已充分)。\n"
            "19. (CHAIN EXAMPLES) 多步工具链典型场景：\n"
            "  a) 路段排放文件 + 用户要扩散 → chain: [\"calculate_macro_emission\", \"calculate_dispersion\"]\n"
            "  b) 路段排放文件 + 用户要扩散 + 用户要热点 → chain: [\"calculate_macro_emission\", \"calculate_dispersion\", \"analyze_hotspots\"]\n"
            "  c) 路段排放文件 + 用户要地图可视化 → chain: [\"calculate_macro_emission\", \"render_spatial_map\"]\n"
            "  d) 用户只要排放因子 → chain: [\"query_emission_factors\"]\n"
            "  e) 用户只要微观排放 → chain: [\"calculate_micro_emission\"]\n"
            "  当 tool_graph 显示工具 A 的 provides 被工具 B 的 requires 匹配时，chain 必须包含 A→B 序列。"
        )
